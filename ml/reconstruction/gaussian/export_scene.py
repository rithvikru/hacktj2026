...
def train_gaussians(
    ...
    """Train 3D Gaussians from point cloud + images.

    Returns path to .splat/.ply output. Falls back gracefully if
    gsplat or gaussian-splatting is unavailable.
    """
...
def _train_with_gsplat(
    ...
    for step in range(min(num_iterations, 1000)):  # Cap iterations for speed
        optimizer.zero_grad()
        # Minimal training step — actual rendering would need camera params
        # For now just optimize positions slightly
        loss = torch.mean(scales.abs())  # regularize scales
        loss.backward()
        optimizer.step()
...
def _train_with_subprocess(
    ...
    """Use graphdeco-inria gaussian-splatting via subprocess."""
    # Look for the gaussian-splatting repo in common locations
    gs_locations = [
    ...
    # Prepare COLMAP-compatible directory structure
...
def _prepare_colmap_scene(
    ...
    """Create a minimal COLMAP-compatible directory for gaussian-splatting."""
    ...
    # Write minimal cameras.txt and images.txt
...
def _pointcloud_to_splat(
    ...
    """Convert point cloud PLY to simple .splat binary format.

    .splat format per gaussian: 3 floats (pos) + 3 floats (scale) +
    4 bytes (rgba) + 4 bytes (quaternion as uint8) = 32 bytes
    """
    points, colors = _load_ply_points(pointcloud_path)
    if len(points) == 0:
        if pointcloud_path.exists():
...
def _load_ply_points(
    ...
    """Load points and colors from a binary little-endian PLY file."""
...
def _save_gaussian_ply(
    ...
    """Save Gaussian parameters as PLY (compatible with viewers)."""
    n = len(means)
