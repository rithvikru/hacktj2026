...
/// Tracks relocalization progress when loading a saved ARWorldMap.
/// Transitions: idle -> relocalizing -> relocalized (or -> failed after 30s timeout).
@Observable
@MainActor
final class RelocalizationMonitor {
    ...
    /// Start monitoring the session manager's tracking state for relocalization.
    ...
    /// Call from tracking state updates to check if relocalization succeeded.
